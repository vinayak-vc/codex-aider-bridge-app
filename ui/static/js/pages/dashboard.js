...
async function _gitGetRepo() {
  if (_gitRepo) return _gitRepo;
  try {
    const s = await fetch('/api/settings').then(r => r.json());
    _gitRepo = s.repo_root || '';
  } catch (_) {}
  return _gitRepo;
}

async function loadGitPanel() {
  const repo = await _gitGetRepo();
  if (!repo) return;
  ...
  try {
    ...
    if (line && !data.error) {
      if (data.is_clean) {
        ...
        if (data.untracked) parts.push(`${data.untracked} untracked`);
        line.innerHTML = `<span style="color:var(--color-warning)">${parts.join(', ')}</span>`;
      }
    }
  } catch (_) {}
  ...
  // Load commits
  try {
    const data = await fetch(`/api/git/log?repo_root=${encodeURIComponent(repo)}&limit=10`).then(r => r.json());
    const list = $('git-commit-list');
    if (list && data.commits?.length) {
      list.innerHTML = data.commits.map(c => {
        const age = _commitAge(c.timestamp);
        const badge = c.is_bridge_task
          ? '<span class="badge badge--accent" style="font-size:9px;margin-left:4px">bridge</span>'
          : '';
        return `
          <div class="git-commit-row" data-sha="${esc(c.sha)}" title="Click to view diff">
            <code class="git-sha">${esc(c.short_sha)}</code>
            <span class="git-commit-msg">${esc(c.message.slice(0, 60))}</span>
            ${badge}
            <span class="git-commit-age">${age}</span>
          </div>
        `;
      }).join('');

      list.querySelectorAll('.git-commit-row').forEach(row => {
        row.addEventListener('click', () => _showCommitDiff(row.dataset.sha));
      });
    }
  } catch (_) {}

  // Load changed files (if dirty)
  try {
    ...
    if (wrap && clist && data.files?.length) {
      clist.innerHTML = data.files.map(f =>
        `<div class="git-changed-file">
...
async function _showCommitDiff(sha) {
  const repo = await _gitGetRepo();
  if (!repo || !sha) return;
  const viewer = $('git-diff-viewer');
  const pre = $('git-diff-pre');
  if (!viewer || !pre) return;

  try {
    const data = await fetch(`/api/git/diff?repo_root=${encodeURIComponent(repo)}&sha=${encodeURIComponent(sha)}`).then(r => r.json());
  ...
  $('git-branch-select')?.addEventListener('change', async e => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        loadGitPanel(); // Reset select to actual branch
      }
    } catch (_) {}
  });
