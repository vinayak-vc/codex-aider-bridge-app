...
- `planner/codex_client.py`
  Builds planner prompts, summarizes idea-file context, resolves Codex CLI execution, captures planner output through a file, and requests plan or retry refinements.
...
- `executor/aider_runner.py`
  Resolves the Aider executable, runs Aider against task-scoped files, and captures stdout, stderr, exit code, and executed command.
...
- `utils/command_resolution.py`
  Resolves external executables from PATH, common local Windows virtual-environment script folders, and explicit relative or absolute command paths.
- `models/task.py`
...
- Resolves `codex` and `aider` commands more robustly across PATH and common local Windows script directories
...
- Retries failed tasks with planner feedback when the planner is available
- Surfaces Aider startup and process failures more clearly in runtime logs
...
- Verified locally for dry-run plan execution from `example plan.json`, project-wide Python compilation, and command resolution for both `codex.cmd` and `aider`
