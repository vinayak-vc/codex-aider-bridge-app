...
const MEMORY_TYPE_NAMES: Record<number, string> = {
  0: 'episodic',
  1: 'semantic',
  2: 'procedural',
};
...
export function registerMemoryTools(server: McpServer): void {
  ...
  server.tool(
    'memory_health',
    'Check whether the bridge-memory-service is running and which storage mode it is in (vector+sqlite or sqlite-only).',
    {},
    async () => {
      try {
        ...
        return ok({
          ...
          url: baseUrl(),
          mode,
          service: (health.data as Record<string, unknown>)?.service ?? 'bridge-memory-service',
        });
      } catch (e) {
  ...
  server.tool(
    'memory_search',
    'Search the bridge memory store for entries relevant to a query. Returns ranked results with scores. Use this at session start (Stage 1.5) to retrieve past run patterns.',
    {
    ...
    async ({ query, limit = 10, type }) => {
      try {
        ...
        const results = (res.data as Array<{ entry: { type: number | string }; score: number }>).map(r => ({
          ...r,
          entry: {
            ...r.entry,
  ...
  server.tool(
    'memory_save',
    'Save a new entry to the bridge memory store. Use at end of session (Stage 5-E) to record what worked, what failed, model preferences, and repo-specific quirks.',
    {
  ...
  server.tool(
    'memory_enhance',
    'Send a task instruction to the memory service for enhancement — relevant past context is injected into the prompt before Aider sees it.',
    {
    ...
    async ({ prompt, limit = 5 }) => {
      try {
        ...
        const enhanced = (res.data as { enhancedPrompt?: string })?.enhancedPrompt ?? prompt;
  ...
  server.tool(
    'memory_ingest',
    'Ingest a completed task result (input instruction + output diff) into the memory store so future sessions can learn from it.',
    {
