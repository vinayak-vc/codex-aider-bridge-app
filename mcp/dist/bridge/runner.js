...
export function detectBridgeRoot() {
    ...
    throw new Error('Could not locate bridge main.py.\n' +
...
function buildArgs(goal, repoRoot, planFile, flags, extra) {
    const args = [
        'main.py', goal,
        '--repo-root', repoRoot,
        '--plan-file', planFile,
    ...
    if (flags.aider_model)
        args.push('--aider-model', flags.aider_model);
    if (flags.validation_command)
        args.push('--validation-command', flags.validation_command);
    if (flags.auto_split_threshold)
        args.push('--auto-split-threshold', String(flags.auto_split_threshold));
    if (flags.manual_supervisor)
        args.push('--manual-supervisor');
    if (flags.aider_no_map)
        args.push('--aider-no-map');
    if (flags.task_timeout)
        args.push('--task-timeout', String(flags.task_timeout));
    if (flags.workflow_profile)
        args.push('--workflow-profile', flags.workflow_profile);
    if (flags.log_level)
        args.push('--log-level', flags.log_level);
    return args;
}
export async function runDryRun(repoRoot, planFile, goal = 'dry-run-check', flags = {}) {
    ...
    return new Promise((resolve) => {
        ...
        proc.on('close', (code) => {
            ...
            for (const line of allLines) {
                ...
                if (m)
                    tasks_preview.push({ id: Number(m[1]), instruction: m[2].trim() });
            }
            // Parse rollback SHA
            let rollback_sha = null;
...
export function startRun(repoRoot, planFile, goal, flags = {}) {
...
// ── log tail ──────────────────────────────────────────────────────────────────
export function tailLog(logFile, lines = 50) {
