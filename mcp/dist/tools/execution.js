...
export function registerExecutionTools(server) {
    // ── bridge_dry_run ──────────────────────────────────────────────────────────
    server.tool('bridge_dry_run', [
    ...
    // ── bridge_cancel ───────────────────────────────────────────────────────────
    server.tool('bridge_cancel', 'Cancel the currently running bridge job (sends SIGTERM to the python main.py process).', {}, async () => {
    ...
    server.tool('bridge_get_run_output', [
        ...
        let latest_task = null;
        const taskRe = /Task (\d+) —/;
        for (const line of tail) {
            const m = line.match(taskRe);
            if (m)
                latest_task = Number(m[1]);
        }
        return ok({
