...
def build_plan_prompt(
    ...
    return (
        ...
        "- Describe the CODE STRUCTURE around the change — is it an array literal,\n"
        "  a function call, a property assignment, a template string? The coding model\n"
        "  will guess wrong if you just say 'replace X with Y' without saying HOW\n"
        "  X appears in the code (e.g., 'the value \"1\" is a standalone array element\n"
        "  on its own line, not an argument to .push()').\n"
        "- Say which value to change, not the whole pattern. E.g., 'change the element\n"
        "  \"1\" on the line after \"--max-videos\"' — not 'replace --max-videos, 1'.\n"
        "- Keep instructions under 200 words. Longer = more tokens = slower.\n\n"
        "BAD instruction: 'Replace the two occurrences of --max-videos, 1 with\n"
        "  the payload value.' Why bad: the model doesn't know if it's .push(\"--max-videos\", \"1\")\n"
        "  or an array literal [\"--max-videos\", \\n \"1\"] — it will guess wrong and fail.\n\n"
        "GOOD instruction: 'In buildUploadCommand(), there are two array literals\n"
        "  containing \"--max-videos\" followed by \"1\" on the next line. Change the\n"
        "  standalone element \"1\" (not the \"--max-videos\" string) to:\n"
        "  String((payload.options && payload.options.maxVideos) || 1)\n"
        "  in both locations (~line 308 and ~line 317).'\n"
        "  Why good: describes the structure (array literal, separate lines), says\n"
        "  exactly which element to change, gives line numbers.\n\n"
        "STRICT RULES:\n"
        ...
        "- Task type must be one of: create, modify, delete, validate, read, investigate, direct\n"
        "- Use type 'direct' for tiny changes where you can write the EXACT file content.\n"
        "  Include the complete file content in a fenced code block in the instruction.\n"
        "  The bridge writes it directly — no LLM involved. Use for: config changes,\n"
        "  adding a single property, simple one-line fixes. Max 1 file, < 50 lines.\n"
