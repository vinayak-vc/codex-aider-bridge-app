...
class TaskParser:
    ...
    def _parse_task(self, item: Any) -> Task:
        ...
        if not isinstance(task_type, str) or task_type not in {"create", "modify", "delete", "validate", "read", "investigate", "direct"}:
            raise PlanParseError(
                f"Task {task_id} has an unsupported type {task_type!r}. "
                "Must be one of: create, modify, delete, validate, read, investigate, direct."
            )
